...
<template>
  ...
  <div class="fixed opacity-0 pointer-events-none -z-10 top-0 left-0" :style="{
    ...
    <template v-for="block in blocks" :key="block.id">
      ...
      <component :is="template.summary" v-else-if="block.type === 'summary'" :ref="setMeasureRef(block.id)"
        :text="resume.summary" />
      <component :is="template.sectionTitle" v-else-if="block.type === 'section-title'" :ref="setMeasureRef(block.id)"
        :section="block.section" />
      <component :is="template.experienceItem" v-else-if="block.type === 'experience-item'"
        :ref="setMeasureRef(block.id)" :item="block.item" />
      <component :is="template.educationItem" v-else-if="block.type === 'education-item'" :ref="setMeasureRef(block.id)"
        :item="block.item" />
      ...
      <div class="absolute top-0 left-0 border border-red-500"
        ...
        <component :is="template.page">
          ...
          <template v-if="page.experience.items.length">
            ...
            <component :is="template.experienceItem" v-for="item in page.experience.items" :key="item.id"
              :item="item" />
          </template>
