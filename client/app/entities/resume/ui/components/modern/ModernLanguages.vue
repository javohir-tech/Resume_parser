...
<template>
  <p class="pb-2 text-[13px] leading-[1.65] break-inside-avoid">
    <strong :style="{ color: resumeStore.designInfo.entry_title_color?.hex }">{{ item.language }}</strong><span v-if="item.degree">: {{ t(`resumeDocument.proficiency.${proficiencyKeys[item.degree]}`) }}</span>
  </p>
</template>
